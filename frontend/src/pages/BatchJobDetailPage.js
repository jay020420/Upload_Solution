...
import {
    ...
    Table, Progress, Timeline, Tooltip, Statistic,
    PageHeader, Space, Spin, Result, Badge, Divider,
    Alert, Modal, message, Typography
} from 'antd';

const { Text } = Typography;

const BatchJobDetailPage = ({ match, history }) => {
    ...
    }, [dispatch, history, jobId, userInfo]);
    ...
    return (
        <div className="batch-job-detail-page">
            <PageHeader
                onBack={() => history.push('/admin/batch-jobs')}
                title="배치 작업 상세"
                subTitle={job?.name}
                extra={[
                    <Button
                ...
                <Result
                    ...
                    extra={[
                        <Button
                            type="primary"
                            key="back"
                            onClick={() => history.push('/admin/batch-jobs')}
                        >
                            목록으로 돌아가기
                        </Button>
                    ]}
